...
DECLARE
  ...
  v_today_done INT := 0;
  v_month_done INT := 0;
  v_month_total_days INT := 0;
  v_total_done INT := 0;
  v_total_total_days INT := 0;
BEGIN
  v_today := COALESCE(p_date, (now() AT TIME ZONE 'America/Sao_Paulo')::date);

  -- Find current month by year + month_number
  ...
  -- HOY: count actual checks for today / 2
  IF v_day_id IS NOT NULL THEN
    SELECT COUNT(*)::int INTO v_today_done
    FROM task_checks tc
  ...
  -- MES: total days in month (ALL days, not just up to today), checks in entire month
  SELECT COUNT(*)::int INTO v_month_total_days
  FROM days d
  JOIN weeks w ON w.id = d.week_id AND w.month_id = v_month_id;

  SELECT COUNT(*)::int INTO v_month_done
  FROM task_checks tc
  JOIN tasks t ON t.id = tc.task_id AND t.is_active = true
  ...
  IF v_month_total_days > 0 THEN
    v_month_pct := LEAST(100, ROUND((v_month_done::numeric / (v_month_total_days * 2)) * 100));
  END IF;

  -- TOTAL: all days in program, all checks in program
  SELECT COUNT(*)::int INTO v_total_total_days
  FROM days d
  JOIN weeks w ON w.id = d.week_id
  JOIN months m ON m.id = w.month_id AND m.program_id = v_program_id;

  SELECT COUNT(*)::int INTO v_total_done
  FROM task_checks tc
  JOIN tasks t ON t.id = tc.task_id AND t.is_active = true
  JOIN days d ON d.id = tc.day_id
  JOIN weeks w ON w.id = d.week_id
  ...
  IF v_total_total_days > 0 THEN
    v_year_pct := LEAST(100, ROUND((v_total_done::numeric / (v_total_total_days * 2)) * 100));
  END IF;
